...
create or replace function public.sync_explicit_project_sections(
  p_project_id text,
...
as $$
begin
  if p_project_data ? 'phases' then
    perform public.sync_normalized_project_schedule(p_project_id, p_project_data);
  end if;
  if p_project_data ?| array['files', 'photos'] then
    perform public.sync_normalized_project_assets(p_project_id, p_project_data);
  end if;
  if p_project_data ? 'selections' then
    perform public.sync_normalized_project_selections(p_project_id, p_project_data);
  end if;
  if p_project_data ? 'inspections' then
    perform public.sync_normalized_project_inspections(p_project_id, p_project_data);
  end if;
  if p_project_data ? 'accessUserIds' then
    perform public.sync_normalized_project_access(p_project_id, p_project_data);
  end if;
end;
$$;

revoke all on function public.sync_explicit_project_sections(text, jsonb) from public, anon, authenticated;
