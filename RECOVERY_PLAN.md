...
No production deploy was locked, restored, published, or unlocked during the initial rehearsal. The server-enforced application write freeze is now implemented and covered by database and client regression tests; the remaining owner decision is who will be the second authorized responder.
...
CI-gated Netlify publishing was implemented in merge commit `6fe8e34`. The current production deploy is locked, so Git-triggered builds cannot auto-publish. After successful push-triggered `main` CI, `.github/workflows/publish-production.yml` finds the ready deploy for the exact tested commit and fixed site, publishes it atomically, verifies both production and deploy URLs, and immediately re-locks it. Operations commits containing `[skip netlify]` are ignored. The Netlify credential is stored only in GitHub's protected `production` environment. The controlled `a39165e` release passed the full gate and workflow rerun `30922207629` verified exact-commit publication, both HTTP checks, and the lock. Bounded retries cover the transient deploy-URL propagation seen on the first verification attempt. Six deterministic tests cover exact commit/site selection, build readiness, URL boundaries, current-deploy selection, retry success, and retry exhaustion.

The application write freeze is implemented as a singleton runtime control with an audited service-role/operator RPC. Statement triggers reject anonymous/authenticated writes across public application tables, restrictive Storage policies reject object mutations, and all three service-role Edge Functions explicitly honor the state. Reads and authorized recovery operations remain available. Current clients poll the state, show a maintenance banner, suppress editing entry points, and pause queued offline mutations; older Android clients are still blocked at the server. The authorization suite proves direct writes and security-definer mutations fail with `APP_WRITES_FROZEN`, reads continue, an otherwise-authorized service-role path bypasses the freeze guard, Storage guards exist, and writes resume after release.

Activation completed on 2026-08-04 after pull request #6 passed protected web, database, and Android checks. Migration `20260804160000_add_application_write_freeze.sql` is in production parity; all three freeze-aware Edge Functions are active; merge commit `ea51756` passed CI run `30926334975`; and exact-commit publisher run `30926517798` verified both URLs and re-locked production. The runtime status remained unfrozen throughout release. No live freeze rehearsal was performed.
